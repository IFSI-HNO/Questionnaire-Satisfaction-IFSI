...
els.type_formation.addEventListener("change", () => {
  ...
  const libelleDetecte = (typeInfo && typeInfo.libelle) || "";
  const concerne = !!typeInfo && TYPES_AVEC_PROMOTION_SEMESTRE.includes(libelleDetecte.trim());

  // --- Promotion : uniquement pour AS/IDE/CDS ---
  els.promotion.innerHTML = "";
  if (!concerne) {
    const opt = document.createElement("option");
    opt.textContent = `Non applicable (libellé détecté : "${libelleDetecte}", longueur ${libelleDetecte.length})`;
    opt.disabled = true;
    opt.selected = true;
